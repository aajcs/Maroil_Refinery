    ...
    <Dropdown
      ...
      itemTemplate={(opt) => (
        <span className="flex items-center gap-2">
          <span className="font-bold">{opt.value.descripcion}</span>
          <span className="text-sm text-gray-600">({opt.value.codigo})</span>
        </span>
      )}
      // ← esto controla cómo se ve el valor seleccionado
      valueTemplate={(opt: any) =>
        opt ? (
          <span className="flex items-center gap-2">
            <span className="font-bold">{opt.value.descripcion}</span>
            <span className="text-sm text-gray-600">({opt.value.codigo})</span>
