	...
	useEffect(() => {
					...
					Object.entries(group).reduce(
						(prev, [productId, despachoGroup]) => {
							...
							Object.entries(
								...
								Object.groupBy(despachoGroup!, (v) => v.fechaLlegada),
							);

							for (const [month, d] of Object.entries(
								// biome-ignore lint/style/noNonNullAssertion: <explanation>
								Object.groupBy(despachoGroup!, (v) =>
									new Date(v.fechaLlegada).getMonth(),
								),
							)) {
								prev[productId][+month] =
									d?.reduce((prev, curr) => prev + curr.cantidadEnviada, 0) ??
									0;
							}
							...
							for (const [month, d] of Object.entries(
								...
								prev[productId][+month] =
									d?.reduce(
										(prev, curr) =>
											prev +
											curr.cantidadEnviada *
												curr.idContratoItems.precioUnitario,
										0,
									) ?? 0;
							}
		...
		fetch(
			...
			.then((body) => {
				...
				setRecepcionesVolumetria(
					Object.entries(
						Object.groupBy(body.recepcions, (d) =>
							new Date(d.fechaFinRecepcion).getMonth(),
						),
					).reduce(
						(prev, [month, curr]) => {
							prev[+month] =
								curr?.reduce((prev, curr) => prev + curr.cantidadEnviada, 0) ??
								0;
				...
				// setRecepciones(body.recepcions);
			})
			.then((_) => {});
		...
		fetch("https://api-maroil-refinery-2500582bacd8.herokuapp.com/api/factura")
			...
			.then((body) => {
				setGastosOperativos(
					Object.values(
						Object.groupBy(body.facturas, (f) => f.idPartida.id ?? "-"),
					).map((f) => ({
						monto: f?.reduce((prev, curr) => prev + curr.total, 0) ?? 0,
						partida: f?.at(0)?.idPartida.descripcion ?? "Sin partida",
						subpartida: f?.at(0)?.idSubPartida?.id ?? "-",
					})),
				);
			});
		...
		fetch("https://api-maroil-refinery-2500582bacd8.herokuapp.com/api/tanque/")
			...
			.then((body) => {
				setInventarios(
					Object.entries(
						Object.groupBy(
							// TODO: obtener solo los tanques de la refineria actual
							body.tanques.filter(
						...
						(prev, [id, group]) => {
							...
							prev[id] = {
								...
								quantity:
									group?.reduce(
										(prev, curr) =>
											prev + (curr.idChequeoCantidad?.cantidad ?? 0),
										0,
									) ?? 0,
							};
			...
			.then((body) => {
				...
				const comprasPorMes = Object.entries(
					Object.groupBy(
						...
						(c) => new Date(c.fechaInicio).getMonth(),
					),
				).reduce(
					(prev, [month, value]) => {
						const total = value?.reduce(
							(prev, curr) => prev + curr.montoTotal,
							0,
						);
			...
			<div className="x">
				<div className="card">
					...
					<div className="volumetriaCardContainer">
						...
						<section
							className={`volumetriaCard ${inventarioVolumetriaOption === 0 && "active"}`}
						...
						{/* biome-ignore lint/a11y/useKeyWithClickEvents: <explanation> */}
						<section
							className={`volumetriaCard ${inventarioVolumetriaOption === 1 && "active"}`}
				...
				<article className="operationalContainer">
					...
					<Chart
						...
						data={{
							labels: Object.keys(
								Object.groupBy(gastosOperativos, (go) => go.partida),
							),
							datasets: [
								{
									label: "Dataset 1",
			...
			<article className="volumetria card">
				...
				<section>
					...
					<div>
						<div className="volumetriaCardContainer">
							{/* biome-ignore lint/a11y/useKeyWithClickEvents: <explanation> */}
							<section
								className={`volumetriaCard ${comprasVolumetriaOption === 0 && "active"}`}
							...
							{/* biome-ignore lint/a11y/useKeyWithClickEvents: <explanation> */}
							<section
								className={`volumetriaCard ${comprasVolumetriaOption === 1 && "active"}`}
				...
				<section>
					...
					<div className="volumetriaCardContainer">
						{/* biome-ignore lint/a11y/useKeyWithClickEvents: <explanation> */}
						<section
							className={`volumetriaCard ${ventasVolumetriaOption === 0 && "active"}`}
						...
						{/* biome-ignore lint/a11y/useKeyWithClickEvents: <explanation> */}
						<section
							className={`volumetriaCard ${ventasVolumetriaOption === 1 && "active"}`}
